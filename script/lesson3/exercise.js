...
class Catalog {
  ...
  fetchGoods() {
    fetch(`${API_URL}/catalogData.json`, {
      ...
      this.goods = data.map(item => new CatalogItem(...item))
      this.render()
    })
...
class Cart {
   ...
   */
  addToBasket(id_product, product_name, price, quantity, image) {
    fetch(`${API_URL}/addToBasket.json`, {
      ...
      .then(
        jsonData => {
          const cartItem = new CartItem(id_product, product_name, price, quantity, image)
          this.products.splice(id_product, 0, cartItem)
          this.countGoods()
          this.countAmount()
        })
  }
